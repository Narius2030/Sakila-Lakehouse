...
def read_rental_payment():
    ...
    payment_df = pl.DataFrame(payment_dataset, infer_schema_length=1000)
    joined_df = rental_df.join(other=payment_df, on="rental_id", how="left")
    
    rental_datasets = joined_df.select(rental_columns).to_dicts()
    payment_datasets = joined_df.select(payment_columns).to_dicts()
    ...
    for rental, payment in zip(rental_datasets, payment_datasets):
        try:
            ...
            # dbopt.insert_dataframe_table_nonconflict("payment", "public", [payment], payment_df.columns, chunk_size=100)
